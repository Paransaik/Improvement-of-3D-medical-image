...
def noiseProcessing(noise, n_raw, batch_size, cnt=0):
    nP = []

    for i in range(batch_size):
        temp = Image.fromarray(noise[n_raw][(cnt*batch_size) + (i+4)])
        temp = np.array(temp.resize((256, 256)))
        nP.append(temp)
...
def build_generator(input_size=(256, 256, 1)):
    ...
    conv4 = BatchNormalization()(conv4) 
    pool3 = MaxPooling2D(pool_size=(2,2))(conv4) # 16
    ...
    conv5 = BatchNormalization()(conv5)  # 16
    ...
    dconv1 = BatchNormalization()(dconv1) # 32
    ...
    dconv2 = BatchNormalization()(dconv2) # 64
    ...
    dconv3 = BatchNormalization()(dconv3) # 128
    ...
    dconv4 = BatchNormalization()(dconv4) # 256
    ...
    dconv5 = Conv2D(depth//2, kernel_size=3, padding='same', activation='relu')(dconv5)
    dconv5 = BatchNormalization()(dconv5) # 512

    # outputs = Conv2D(1, (1, 1), activation='sigmoid')(dconv5)
    outputs = Conv2D(1, (1, 1), activation='sigmoid')(dconv5) # sigmoid 안한다면 hr_imgs와 값이 맞다.
...
def build_discriminator():
    ...
    model.add(Dense(1)) # 판별자의 마지막 층 시그모이드 함수 제거 
...
discriminator.compile(loss=wasserstein, # mse도 생각해보기
                      optimizer=RMSprop())
discriminator.trainable = False

generator = build_generator()
generator.compile(loss=wasserstein, optimizer=RMSprop())
generator.summary()
...
dcgan = build_gan(generator, discriminator)
dcgan.compile(loss=wasserstein, optimizer=RMSprop())
...
def train(iterations, batch_size, sample_interval, clip_threshold):
    # generator.load_weights('gan_g_weights.h5')
    # discriminator.load_weights('gan_d_weights.h5')
    ...
    for iteration in range(iterations): # Epoch라고 생각하면 될 듯 
        ...
        for n_raw, hr_imgs in enumerate(train_y): # hr_imgs = (channel, 512, 512)
            ...
            while(1):
                ...
                for i in range(2):
                    ...
                    d_loss = 0.5 * np.add(d_loss_real, d_loss_fake) # 판별자의 손실은 real_loss와 gen_loss로 이루어진다 real이미지와의 결과가 1에 가까울 수록 loss가 줄어들고 gen이미지와의 결과가 0에 가까울 수록 loss가 줄어들도록한다., 0.5를 곱하는 것은 손실값을 줄여 학습 파라미터가 크게 날뛰는 걸 방지하는 용도
        ...
        if (iteration + 1 ) % sample_interval == 0:
            ...
            print("%d [D loss: (%.3f)(R %.3f, F %.3f)]  [G loss: %.3f] " % (iteration+1, d_loss, d_loss_real, d_loss_fake, g_loss))
...
train(iterations=10, batch_size=1, sample_interval=1, clip_threshold=0.01) # batch_size는 20으로 고정해야함
