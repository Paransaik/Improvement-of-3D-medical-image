...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        # Qt는 메뉴, 툴바, 상태바 등을 편리하게 생성하고 관리할 수 있는 액션(QAction)이라는 개념을 도입
        
        openAction = QAction(QIcon('open.png'), 'Open', self) # 왼쪽 위 메뉴위치에 아이콘 설정
        openAction.triggered.connect(self.openImage) # 클릭하면 연결된 openImage 메소드가 실행 
        self.toolbar = self.addToolBar('Open')
        self.toolbar.addAction(openAction)

        # 푸시 버튼 또는 명렁 버튼은 사용자가 프로그램에 명령을 내려서 어떤 동작을 하도록 할 때 사용되는 버튼
        Dbtn = QPushButton('&ImgNum', self) # &은 단축키를 지정하기 위해서 첫 번째 파라미터는 버튼에 나타날 텍스트, 두 번째 파라미터는 버튼이 속할 부모 클래스
        Dbtn.move(900, 565) # 위젯을 스크린의 x=900px, y=300px의 위치로 이동
        Dbtn.setCheckable(True) # 누른 상태와 그렇지 않은 상태를 구분한다.
        Dbtn.toggle() # 버튼의 상태가 바뀌게 된다. 따라서 이 버튼은 프로그램이 시작될 때 선택되어 있다.
        Dbtn.clicked.connect(self.showDialog) # 버튼 클릭시 연결될 메소드
        ...
        btn1.clicked.connect(self.btn1_clicked) # 각 버튼 클릭시에 연결될 메소드
        ...
        self.setGeometry(300, 300, 1100, 600) # (move, resize)의 기능을 넣음, resize(w, h) : 위젯의 크기를 너비 w(px), 높이 h(px)로 조절 

        self.show()

    def showDialog(self): # ImgNum 버튼 클릭시
    ...
    def btn3_clicked(self):
        ...
        Btn3.VolumeRendering(self.folder_path) # Volume
    ...
    def openImage(self):
        ...
        self.imagePath, _ = QFileDialog.getOpenFileName(self, 'Open file', './Opentool_img/') # 'Open file'은 열리는 위젯의 이름, 세 번째 매개변수는 기본 경로설정
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.LRClicked: # mousePressEvent에서 클릭을 감지하면 True로 변경
    ...
    def mousePressEvent(self, event): # CT 이미지의 width 변경?! 제대로 된 확인이 안된다면 test.py에서 구현해보기
       ...
       # LeftButton, RightButton, MiddleButton의 조합으로 자주 사용 OR 연산자를 사용
        ...
        if event.buttons() == QtCore.Qt.LeftButton | QtCore.Qt.RightButton: # 질문 : 이게 왜 동시 클릭을 의미?
            ...
            else: # 이미 LRClicked가 True(클릭)으로 되어있다면, 아마 마우스 버튼 클릭 후 떼는 동작!
