...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        btn1.move(500, 565) # 위젯을 스크린의 x=900px, y=300px의 위치로 이동
        ...
        btn5 = QPushButton('&ImgNum', self)
        btn5.move(900, 565)
        btn5.setCheckable(True)
        btn5.toggle()

        btn3.setShortcut('Ctrl+1')
        btn4.setShortcut('Ctrl+2')

        btn1.clicked.connect(self.stopButton) # 버튼 클릭시 연결될 메소드
        btn2.clicked.connect(self.s_rButton)
        btn3.clicked.connect(self.previousButton)
        btn4.clicked.connect(self.nextButton)
    ...
    def openImage(self):
        ...
        # C:/Users/haram/PycharmProjects/ConnectAi/viewer/image/001.Seq1.Ser5.Img1.dcm

        # test = QFileDialog.getOpenFileName(self, 'Open file', './image')[0]
        # print("open", test)

        # C:/Users/haram/PycharmProjects/ConnectAi/viewer/image/001.Seq1.Ser5.Img1.dcm

        # test =  imagePath.split('/')[-1]
        # print(test)
        # .dcm이 있는 폴더위치로 path를 지정해주니 된다.
        folder_path = "C:/Users/haram/PycharmProjects/ConnectAi/viewer/image" # home
        # folder_path = "/Users/taeyeong/PycharmProjects/ConnectAi/viewer/image" # Mac
        # folder_path = "C:/ConnectAi/viewer/image" # 403

        #filename = imagePath.split('/')[-1]
        #print(filename)

        reader = itk.ImageSeriesReader() # reader이름의 객체 생성
        dicom_names = reader.GetGDCMSeriesFileNames(folder_path) # 폴더내에있는 .dcm 파일을 가져온다.
    ...
    def saveImage(self):
        ...
        ImgArray = self.vx.m_Voxel

        #vx.PyVoxel.ReadFromBin(self, filename)
        #vx.PyVoxel.WriteToRaw(self, filename)
        #vx.PyVoxel.WriteToBin(self, filename)
        #vx.PyVoxel.SaveWithoutHeader(self, filename)
    ...
    def previousButton(self):
        ...
        self.wg.view_2.setScene(self.wg.lbl_original_img) # wh.lbl_blending_img는 어디갔찌 < 유성이 귀엽노
        self.wg.view_1.show()
        self.wg.view_2.show()

    # btn4(next)가 클릭 되었을 때
    def nextButton(self):
